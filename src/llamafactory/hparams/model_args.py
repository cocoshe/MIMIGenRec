...
class BaseModelArguments:
    ...
    add_tokens_list: str | None = field(
        default=None,
        metadata={
            "help": (
                "Non-special tokens to be added into the tokenizer."
                "If set, `add_tokens` will be ignored."
                "JSON format: ['tok1', 'tok2', 'tok3']"
            )
        },
    )
    ...
    def __post_init__(self):
        ...
        if self.add_tokens_list is not None:
            with open(self.add_tokens_list) as f:
                self.add_tokens = json.load(f)
        elif self.add_tokens is not None:  # support multiple tokens
