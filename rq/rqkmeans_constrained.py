...
def deal_with_deduplicate(df):
    ...
    result_df = df_with_index.with_columns(
        ...
        .then(pl.col("codes").list.concat(pl.col("index").rank(method="ordinal").over("codes").cast(pl.Int64)))
...
if __name__ == "__main__":
    ...
    # Load data
    print("=" * 60)
    print("RQ-KMeans Constrained Training")
    print("=" * 60)
    ...
    codes_all, codebooks, recon = residual_kmeans_constrained(
        embeddings, K=K_values, L=args.l, random_state=args.seed, verbose=args.verbose, max_iter=args.max_iter
    )
    ...
    np.savez_compressed(codebook_path, **{f"codebook_{i}": cb for i, cb in enumerate(codebooks)})
    ...
    duplicates = codes_str.group_by("codes_str").count().filter(pl.col("count") > 1).sort("count", descending=True)
