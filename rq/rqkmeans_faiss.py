...
def sinkhorn_balance_level(
    residuals, centroids, capacities=None, *, batch_size=8192, iters=30, tau=None, verbose=True, topk=32, seed=42
):
    import ot
    ...
    if verbose:
        print(f"  Sinkhorn level: N={N}  K={K}  tau={tau:.5g}  iters={iters}  batch={batch_size}")
...
def sinkhorn_uniform_mapping(rq, data, codes, *, batch_size=8192, iters=30, tau=None, verbose=True, topk=32, seed=42):
    codebooks = get_rq_codebooks(rq)
    N, M = codes.shape
    K = codebooks.shape[1]
    ...
    for l in range(M):
        ...
        residuals = compute_residuals_upto_level(rq, data, codes_bal, upto_level=l, codebooks=codebooks)
...
def analyze_codes(codes, title="", verbose=True):
    ...
    if verbose:
        ...
        print(f"  unique full-paths={combos}  collision_rate={1 - combos / N:.4f}")
    return
...
def main():
    ...
    parser.add_argument("--iters", type=int, default=30, help="Sinkhorn iterations")
    ...
    if args.uniform:
        codes_bal = sinkhorn_uniform_mapping(
            rq, data, codes_raw, batch_size=args.batch_size, iters=args.iters, verbose=True
        )
